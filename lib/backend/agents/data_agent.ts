...
// ---------------------------------------------------------------------------
// Cell Value Cache — (제품명, 기준) 쌍의 Tavily 판단 결과를 영구 저장
// 동일한 제품+기준 조합은 항상 같은 값을 반환하도록 결정론적 보장
// ---------------------------------------------------------------------------
...
type CellCache = Record<string, CellCacheEntry>;

const CELL_CACHE_PATH = path.join(process.cwd(), "data", "cell-cache.json");

function loadCellCache(): CellCache {
  try {
    if (fs.existsSync(CELL_CACHE_PATH)) {
      return JSON.parse(fs.readFileSync(CELL_CACHE_PATH, "utf8")) as CellCache;
    }
  } catch { /* ignore */ }
  return {};
}

function saveCellCache(cache: CellCache): void {
  try {
    fs.mkdirSync(path.dirname(CELL_CACHE_PATH), { recursive: true });
    fs.writeFileSync(CELL_CACHE_PATH, JSON.stringify(cache, null, 2), "utf8");
  } catch { /* ignore */ }
}

function makeCacheKey(productName: string, criterion: string): string {
  return `${productName.trim()}||${criterion.trim().replace(/\s*\[.*?\]\s*/g, "").trim()}`;
}
...
async function tavilySearch(
  ...
  const res = await fetch("https://api.tavily.com/search", {
    ...
    body: JSON.stringify({ query, search_depth: searchDepth, max_results: 5, include_answer: false }),
...
async function judgeCell(
  ...
  const keywords = [criterion, ...synonyms, ...criterion.split(/\s+/), productName].filter(w => w.length >= 2);
...
export async function enrichCompTableCells(
  ...
  // 쮨 병렬 검색 전, 쫚8 존재 여부 확인
  const needsTavily = allCellsToVerify.filter(({ rowCriterion, productLabel }) => {
    ...
    if (cellCache[cacheKey]) {
      ...
      if (targetRow) {
        const productCols2 = ((tableJson.props?.columns ?? []) as any[]).filter((c: any) => c.key !== "criterion");
        const col2 = productCols2.find((c: any) => c.label === productLabel);
        if (col2) {
          (targetRow as Record<string, string>)[col2.key] = cellCache[cacheKey].value;
          cacheHits++;
          console.log(`\x1b[36m[Cache HIT] "${productLabel}" × "${rowCriterion}" → "${cellCache[cacheKey].value}"\x1b[0m`);
        }
      }
      return false; // Tavily 불필요
    }
    return true; // Tavily 필요
  });
  ...
  if (needsTavily.length === 0) {
    ...
    await Promise.all(
      needsTavily.map(async ({ rowCriterion, colKey, productLabel }) => {
        ...
        // 마지막 저장: non-"-" 값만 쫚8에 저장
        if (value !== "-") {
          const fullName = fullNameMap.get(productLabel) ?? productLabel;
          const cacheKey = makeCacheKey(fullName, rowCriterion);
          cellCache[cacheKey] = { value, sourceUrl, usedSnippet, cachedAt: new Date().toISOString() };
        }
      })
    );
    ...
    // 모든 Tavily 결과 캐시 저장
    saveCellCache(cellCache);
    console.log(`\x1b[36m[Cache] cell-cache.json 저장 완료\x1b[0m`);
  }
}
...
export async function reRankByAI(
  ...
  const { text } = await generateText({
    model: anthropic("claude-haiku-4-5"),
    system: `You are a strict product selector. You receive a numbered list of REAL products from a Korean price comparison site, each with a spec list.
Your job: select up to ${targetCount} products that satisfy ALL of the given criteria.
...
3. AND RULE: A product MUST satisfy EVERY criterion in the criteria list to be selected.
   - For each criterion, find that feature (or a clear synonym) in the product's Specs text.
   - If even ONE criterion is NOT confirmed in Specs, EXCLUDE the product entirely.
4. NUMERIC RULE: If a criterion includes a numeric threshold (e.g., "3000Pa 이상"):
   - Find the relevant spec field and compare numerically.
   - Only select the product if its value meets or exceeds the threshold.
5. If ZERO products satisfy ALL criteria, output exactly: [{"index": -1, "reason": "NO_FEATURE_IN_DB", "appliedCriteria": []}]
6. Otherwise, output a valid JSON array: [{"index": N, "reason": "...", "appliedCriteria": ["기준1", "기준2"]}]
   - "reason": brief spec evidence confirming ALL criteria in Korean (1 sentence).
   - "appliedCriteria": list of ALL criteria confirmed in this product's specs.
7. Select at most ${targetCount} products. All selected products MUST satisfy every criterion.
...
Output JSON array only.`,
  ...
  try {
    ...
    console.warn("\x1b[31m[reRankByAI] JSON parse failed. Claude raw response:\x1b[0m");
    console.warn(text.slice(0, 500));
    return RAG_NOT_FOUND;
  }

  if (parsed.length === 1 && parsed[0]?.index === -1 && parsed[0]?.reason === "NO_FEATURE_IN_DB") {
    console.warn("\x1b[33m[AI Reranker] DB에 해당 기능을 가진 제품 없음 → NOT_FOUND 반환\x1b[0m");
    return RAG_NOT_FOUND;
  }
  ...
  const validated: RankedProduct[] = parsed
    ...
    .map((r) => ({
      ...
      appliedCriteria: Array.isArray(r.appliedCriteria) ? r.appliedCriteria.map(String) : [],
    }));

  if (validated.length === 0) {
    console.warn("\x1b[33m[AI Reranker] 유효한 인덱스 0개 → NOT_FOUND 반환\x1b[0m");
    return RAG_NOT_FOUND;
  }
...
export async function computeRankingAndReasoning(
  ...
  try {
    const { text: reasoning } = await generateText({
      model: anthropic("claude-haiku-4-5"),
      system: `You are a friendly shopping advisor helping a first-time buyer understand a product comparison table.
STRICT RULES:
- Base your analysis ONLY on the table data provided. Do NOT add any information not in the table.
- If all products are tied (공동 순위), acknowledge they are equal on these criteria and suggest what else to consider.
- If products differ, explain the key differences using specific table values (e.g., "○", "X", actual numbers).
- Never mention scores, weights, WSM, or formulas.
- Do NOT use any markdown formatting: no **, *, #, _, or bullet points. Plain text only.
- Write 2-3 sentences in ${lang}.`,
      prompt: `Ranking result: ${actualRankLines}\n\nFull comparison table:\n${fullTableLines}\n\nWrite a warm, factual analysis of these comparison results for a first-time buyer.`,
      temperature: 0.3,
    });

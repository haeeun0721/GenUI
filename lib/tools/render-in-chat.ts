...
import { currentRequestId, pushChatUIResult, currentUserContext, currentSavedItems, currentDecisionCriteria, currentMyItemsContextSummary } from "./sidebar-store";
...
export const renderInChat = tool({
    ...
    inputSchema: z.object({
        ...
        ui_intent_category: z.string().nullable().describe("Category of intent: '2' (Comparison), '3' (Recommendation), '4' (Spec Interpretation)."),
    }),
    execute: async ({ ui_context, intent_summary, ui_intent_category }) => {
        console.log([
            "[Tool: renderInChat] OUTPUT FORMAT",
            `  category     : ${ui_intent_category}`,
            `  intent       : ${intent_summary}`,
            `  ui_context   : ${ui_context}`,
        ].join("\n"));
        // If the Conversation Agent sent the pre-fetch signal, swap in the real data from the store
        const resolvedContext = ui_context.trim() === "[MY ITEMS REQUESTED]"
            ? currentMyItemsContextSummary
            : ui_context;

        try {
            const uiSpecString = await generateUISpec(resolvedContext, intent_summary, ui_intent_category, 1, currentUserContext, currentSavedItems, currentDecisionCriteria);
            if (uiSpecString && !uiSpecString.startsWith("ERROR:")) {
                ...
                if (firstBrace !== -1) {
                    ...
                    if (lastBrace !== -1) {
                        ...
                        if (uiSpec.props?._rankReasoning) {
                            console.log("\n\x1b[36m[순위 산정 이유 (WSM)]\x1b[0m\n" + uiSpec.props._rankReasoning + "\n");
                        }
